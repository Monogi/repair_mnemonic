...
def parse_derivation_path(str_derivation_path):
    ...
    if str_derivation_path[0:2] != 'm/':
        raise ValueError("Can't recognize derivation path. It should look like \"m/44'/60/{account}'/0\".")
...
def mnemonic_to_eth(mnemonic_words: str, account_number: int = 0, address_index: int = 0):
    # mnemonic_words ="acquire slight alone south similar nation noble change mass grass salmon float coyote hip potato"
    private_key = mnemonic_to_private_key(mnemonic_words,
                                          str_derivation_path=f"m/44'/60'/{account_number}'/0/{address_index}")
    public_key = PublicKey(private_key)
    return {"private_key": binascii.hexlify(private_key).decode("utf-8"),
...
if __name__ == '__main__':
    ...
    #test mnemonic
    mnemonic = "acquire slight alone south similar nation noble change mass grass salmon float coyote hip potato"
